...
### Рішення
В завданні йдеться про /etc/shadow, але у FreeBSD файл з хешами паролів це /etc/master.passwd, тому будемо використовувати його.

"конфігурація системи дозволяє отримувати адміністративний доступ за допомогою відповідної команди" - тут йдеться про sudo, і як виявилось, в мене він не встновлений.
Тож спочатку встановлюємо sudo командою:
```
pkg install sudo
```

Далі відкриваємо файл sudoers командою:
```
visudo
```
Додаємо в файл команду, яка дозволить нам виконати команду із завдання:
```
anastasiia ALL=(ALL) NOPASSWD: /bin/cat /etc/master.passwd
```

В програмі викликаємо system("sudo cat /etc/master.passwd"), отримуємо такий результат:

![image](https://github.com/user-attachments/assets/c84efd3c-12d9-478d-81a0-f442ad8041cd)
...
### Рішення
Спочатку проограма визначає домашній каталог поточного користувача за допомогою getpwuid(getuid()), формує шлях до нового файлу user_file.txt, відкриває його у режимі запису ("w"), записує тестовий рядок і закриває файл — таким чином, звичайний користувач створює файл у своїй домашній теці з власними правами.

Далі якщо програму запущено з UID 0 (тобто від імені root), вона виконує системну команду, яка копіює створений файл у той самий каталог під новим іменем, змінює власника на root:wheel і встановлює права 644, щоб заборонити зміну вмісту звичайним користувачам, після чого виходить.

Після копіювання програма знову працює від імені звичайного користувача й намагається відкрити файл на дозапис ("a").

Після спроби копіювання відбувається спроба видалення.

І тут є цікавий момент: експектед біхейтор (смотря какой фабрік) цієї програми, це permission denied на редагування та видалення копії файлу звичайним користувачем. 
Але оскільки під час виконання попереднього завдання я змінювала файл sudoers, я там також розкоментувала деякі рядки, чим дозволила повний доступ без пароля для групи wheel, а мій користувач належить до цієї групи. Тож мені все вдалося, результат вийшов наступним:

![image](https://github.com/user-attachments/assets/6b14174a-8a39-47d8-abe0-e6ddadb36098)
